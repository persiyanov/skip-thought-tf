...
@click.command()
@click.option('--data_path', default='data/input.txt', help="Path to file with text data.")
@click.option('--save_dir', default='save/', help='Directory to store checkpointed models')
@click.option('--num_epochs', default=10, help="Number of epochs.")
@click.option('--num_layers', default=1, help="Number of layers in encoder.")
@click.option('--batch_size', default=128, help="The size of batch.")
@click.option('--num_hidden', default=512, help="Hidden size of the cell.")
@click.option('--cell_type', default='gru',
              type=click.Choice(SkipthoughtModel.SUPPORTED_CELLTYPES),
              help='Type of the RNN cell.')
@click.option('--embedding_size', default=1024, help="The size of word embeddings.")
@click.option('--max_vocab_size', default=100000, help="Size of vocabulary. Most frequent words are used.")
@click.option('--num_samples', default=512, help="Number of samples in sampled softmax.")
@click.option('--learning_rate', default=0.01, help="Initial learning rate.")
@click.option('--decay_rate', default=0.99, help="Exponential decay rate.")
@click.option('--grad_clip', default=5.0, help="Value for gradient clipping.")
@click.option('--save_every', default=1000, help="Number of batch steps before creating a model checkpoint")
...
def main(**kwargs):
    ...
    with tf.Session() as sess:
        ...
        for e in range(kwargs['num_epochs']):
            it = textdata.triples_data_iterator(*triples, textdata.max_len, kwargs['batch_size'])
